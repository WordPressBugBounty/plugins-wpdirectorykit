    ...
    <div id="wrapper" dir="ltr" style="max-width: 600px; width:calc(100% - 30px); background-color: #fff; margin: 0 auto;border: 1px solid #dedede;box-shadow: 0 1px 4px rgb(0 0 0 / 10%);">
        ...
        <?php foreach ($chats as $key => $chat) : ?>
            <h2 style="margin-top:0;color:#000">
                <?php echo esc_html__('New Messsages from', 'wpdirectorykit').' ' . esc_html(wmvc_show_data('display_name', $chat['user_from'], '-')).'('.esc_html($chat['count']).') : <a href="' . esc_url(wdk_dash_url('dash_page=messages&function=chat&id='.wmvc_show_data('idmessage', $chat['inbox'], '-'))) . '">' . esc_html__('Check Chat', 'wpdirectorykit') . '</a>'; ?>
            </h2>
            <p> <?php echo esc_html__('Latest messages', 'wpdirectorykit');?>:</p>
            <table style="border: 1px solid;border-collapse: collapse;width:100%;">
                <thead>
                    <tr>
                        <th style="padding:10px 10px; border: 1px solid;border-collapse: collapse;text-align: left;"><?php echo esc_html__('Message', 'wpdirectorykit'); ?></th>
                        <th style="padding:10px 10px; border: 1px solid;border-collapse: collapse;text-align:center"><?php echo esc_html__('Date', 'wpdirectorykit'); ?></th>
                    </tr>
                </thead>
                <tbody>
                    <?php foreach ($chat['messages'] as $key => $message) : if($key>=3) break; ?>
                        <tr>
                            <td style="padding:10px 10px; border: 1px solid;border-collapse: collapse;">
                                <?php echo esc_html(wmvc_show_data('message', $message, '-')); ?>
                            </td>
                            <td style="padding:10px 10px; border: 1px solid;border-collapse: collapse;text-align: center;">
                                <?php echo esc_html(wdk_get_date(strtotime($message->date))); ?>
                            </td>
                        </tr>
                    <?php endforeach; ?>
                </tbody>
