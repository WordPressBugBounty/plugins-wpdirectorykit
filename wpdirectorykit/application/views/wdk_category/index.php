...
<div class="wrap wdk-wrap">
    <h1 class="wp-heading-inline"><?php echo esc_html__('Categories','wpdirectorykit'); ?> <a href="<?php echo esc_url(get_admin_url() . "admin.php?page=wdk_category&function=edit"); ?>" class="button button-primary" id="add_category_button"><?php echo esc_html__('Add Category','wpdirectorykit'); ?></a></h1>
    <br />
    <?php
    ...
    <form method="GET" action="<?php echo esc_url(wmvc_current_edit_url()); ?>" novalidate="novalidate">
        <table class="wp-list-table widefat fixed striped table-view-list pages">
            ...
            <?php if(count($categories) == 0): ?>
                <tr class="no-items"><td class="colspanchange" colspan="6"><?php echo esc_html__('No Categories found.','wpdirectorykit'); ?></td></tr>
            <?php endif; ?>
            <?php foreach ( $categories as $category ):?>
                <tr>
                    <th scope="row" class="check-column">
                        <input id="cb-select-<?php echo esc_attr(wmvc_show_data('idcategory', $category, '-')); ?>" type="checkbox" name="post[]" value="<?php echo esc_attr(wmvc_show_data('idcategory', $category, '-')); ?>">
                    ...
                    <td>
                        <?php echo esc_html(str_pad('', wmvc_show_data('level', $category, 0)*12, '&nbsp;')).'|-'; ?><a href="<?php echo esc_url(get_admin_url() . "admin.php?page=wdk_category&function=edit&id=".wmvc_show_data('idcategory', $category, '-')); ?>"><?php echo esc_html(wp_strip_all_tags(wmvc_show_data('category_title', $category, '-'))).' #'.esc_html(wmvc_show_data('idcategory', $category, '-')); ?></a>
                    </td>
                    <td>
                        <?php echo esc_html(wmvc_show_data('order_index', $category, '-')); ?>
                    </td>
                    <td>
                        <?php echo esc_html(wmvc_show_data('level', $category, '-')); ?>
                    </td>
                    <td>
                        <?php echo esc_html(wdk_get_date(wmvc_show_data('date', $category), false)); ?>
                    </td>
    ...
    <div class="alert alert-info" style="margin-bottom:20px" role="alert"><?php
        echo wp_kses(
            sprintf(
                ...
                __('%1$s How to manage Categories documentation%2$s', 'wpdirectorykit'),
                '<a href="' . esc_url('https://wpdirectorykit.com/documentation/#!/categories') . '" target="_blank">',
                '</a>'
            ),
            array(
                'a' => array(
